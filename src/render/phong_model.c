...
static t_color add_all_phong_colors(t_phong phong, t_data data)
{
	t_color phong_color;

	phong_color = apply_brightness(phong.ambient_color, data.ambient.ratio);
...
t_color	apply_phong_model(t_data data, t_coordinate hit_point)
{
	...
	color_rgb = add_all_phong_colors(phong, data);
	return (color_rgb);
}

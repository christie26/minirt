...
t_coordinate hit_cylinder_lid(t_cylinder cylinder, t_ray *ray, t_coordinate base_center, t_coordinate top_center)
{
    double          t1, t2;
...
t_vector	get_cylinder_normal(t_cylinder cylinder, t_coordinate hit_point)
{
	...
	if (vector_dot(get_vector_two_point(hit_point, cylinder.base), cylinder.vector) == 0)
		normal = vector_unit(vector_mult_scalar(cylinder.vector, -1));
	else if (vector_dot(get_vector_two_point(hit_point, cylinder.top), cylinder.vector) == 0)
		normal = vector_unit(cylinder.vector);
	else
	{
		v = get_vector_two_point(cylinder.base, hit_point);
		p = vector_add(coordinate_to_vector(cylinder.base),vector_mult_scalar(cylinder.vector, vector_dot(v, cylinder.vector)));
...
t_coordinate get_side_point(t_hit_cylinder info, t_ray ray, t_cylinder cylinder, double height)
{
	double			check_1;
	double			check_2;
